##- Exercici 1
#La teva tasca és dissenyar i crear una taula anomenada "credit_card" que emmagatzemi detalls crucials sobre les targetes de crèdit. La nova taula ha de ser capaç 
#d'identificar de manera única cada targeta i establir una relació adequada amb les altres dues taules ("transaction" i "company"). 
...
#Ara importo les dades des de l'arxiu sql:

-- Insertamos datos de credit_cardS
...
# Cambiamos el tipo de dato de expiring_date:
UPDATE credit_card
...
# Això dona error per que no existeix ni a la taula company ni a la taula credit_card
...
SELECT company_name as 'Nom de la companyia', 
	phone as 'Telèfon de contacte', 
    country as 'País de residència', 
    AVG(amount) as 'Mitjana de la compra'
...
#Esto da error por haver usado alias con caràcters especials, ho solucionem demanant a la query que faci servir la mateixa cofiguració per comparar les string:

SELECT *
FROM vistamarketing_1
WHERE CONVERT(`País de residència` USING utf8) = CONVERT('Germany' USING utf8);
...
# Segons el diagrama el primer que va fer es introduir una altra taula: user
...
# No es pot establir una relació entre les taules per que hi ha un valor a la taula transaction que a user no hi és per que l'hem afeigit manualment en l'exercici anterior
# És el id= 9999, per tant, l'hem d'afegir a la taula 'user'.
...
#canviem el tipus de dada de 'cvv':

ALTER TABLE credit_card
MODIFY COLUMN cvv INT;

# canviem el tipus de dada a expiring_date:

ALTER TABLE credit_card
MODIFY COLUMN expiring_date VARCHAR(10);


# afegim camp fecha_actual:

ALTER TABLE credit_card
ADD fecha_actual DATE;

# Crear i omplir el camp de fecha_actual amb la data actual:

ALTER TABLE credit_card
ADD fecha_actual_2 TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

UPDATE credit_card
SET fecha_actual_2 = CURRENT_TIMESTAMP
WHERE fecha_actual IS NULL;


#Canviem el nom del camp email a personal_email:
